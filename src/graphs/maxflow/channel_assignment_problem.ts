...
export class Graph {
  ...
  bpm(table: number[][], u: number, seen: boolean[], matchR: number[]): boolean {
    for (let v = 0; v < this.N; v++) {
      if (table[u][v] > 0 && !seen[v]) {
        seen[v] = true;

        if (matchR[v] < 0 || this.bpm(table, matchR[v], seen, matchR)) {
          matchR[v] = u;
          return true;
        }
      }
  ...
  max_bmp(table: number[][]): number {
    const matchR: number[] = Array(this.N).fill(-1);
    let result = 0;

    for (let u = 0; u < this.M; u++) {
      const seen: boolean[] = Array(this.N).fill(false);
      if (this.bpm(table, u, seen, matchR)) {
        result += 1;
      }
    }

    console.log("The number of maximum packets sent in the time slot is", result);
    for (let x = 0; x < this.N; x++) {
      if (matchR[x] + 1 !== 0) {
        console.log(`T${matchR[x] + 1}-> R${x + 1}`);
      }
    }
...
if (require.main === module) {
  ...
  const max_flow = gr.max_bmp(table);
}
