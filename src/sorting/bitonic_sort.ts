...
function compAndSwap(a: number[], i: number, j: number, dire: number): void {
    if ((dire === 1 && a[i] > a[j]) || (dire === 0 && a[i] < a[j])) {
      [a[i], a[j]] = [a[j], a[i]];
    }
  }
  
  function bitonicMerge(a: number[], low: number, cnt: number, dire: number): void {
    if (cnt > 1) {
      const k: number = cnt / 2;
  ...
  function _bitonicSort(a: number[], low: number, cnt: number, dire: number): void {
    if (cnt > 1) {
      const k: number = cnt / 2;
      _bitonicSort(a, low, k, 1);
      _bitonicSort(a, low + k, k, 0);
      bitonicMerge(a, low, cnt, dire);
    }
  }
  
  function bitonicSort(a: number[], N: number, up: number): void {
    _bitonicSort(a, 0, N, up);
  }
  if (require.main === module) {
      // Example usage:
    const a: number[] = [3, 7, 4, 8, 6, 2, 1, 5];
