...
function kLargest(arr: number[], k: number): void {
...
function firstKElements(arr: number[], size: number, k: number): void {
    ...
    for (let i = k; i < size; i++) {
        // If the current element is smaller than the minimum element, do nothing and continue to the next element
        if (minHeap.top() > arr[i]) {
            continue;
        }

        // Otherwise, change the minimum element to the current element and restore the heap property
...
if (require.main === module) {
    const arr: number[] = [1, 23, 12, 9, 30, 2, 50];
    const k: number = 3;
