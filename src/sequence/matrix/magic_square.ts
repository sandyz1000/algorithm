...
function generateSquare(n: number): void {
  const magicSquare: number[][] = Array.from({ length: n }, () =>
    Array.from({ length: n }, () => 0)
  );
  let i: number = Math.floor(n / 2); // initialize position of 1
  let j: number = n - 1;
  ...
  console.log("Magic Square for n =", n);
...
if (require.main === module) {
  let n: number = 7; // Works only when n is odd
  n = 3;
  generateSquare(n);
}